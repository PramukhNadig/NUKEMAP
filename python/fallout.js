...
function fallout() {
  ...
  this.SFSS_fallout_params = function (kt) {
    ...
    if (kt >= 9) {
      var z_o = (1900 + (alpha_2_3 + 0.02) * z_s) / alpha_2_3;
    } else {
      var z_o = h - b;
    }
    ...
    var phi_5 =
      (alpha_5 + a_h + Math.sqrt(a_b_2 + pow(alpha_5 + a_h, 2))) /
      (alpha_5 - a_h + Math.sqrt(a_b_2 + pow(alpha_5 - a_h, 2)));
    var phi_6 =
      (alpha_6 + a_h + Math.sqrt(a_b_2 + pow(alpha_6 + a_h, 2))) /
      (alpha_6 - a_h + Math.sqrt(a_b_2 + pow(alpha_6 - a_h, 2)));
    var phi_7 =
      (alpha_7 + a_h + Math.sqrt(a_b_2 + pow(alpha_7 + a_h, 2))) /
      (alpha_7 - a_h + Math.sqrt(a_b_2 + pow(alpha_7 - a_h, 2)));
    var phi_8 =
      (alpha_8 + a_h + Math.sqrt(a_b_2 + pow(alpha_8 + a_h, 2))) /
      (alpha_8 - a_h + Math.sqrt(a_b_2 + pow(alpha_8 - a_h, 2)));
    var phi_9 =
      (alpha_9 + a_h + Math.sqrt(a_b_2 + pow(alpha_9 + a_h, 2))) /
      (alpha_9 - a_h + Math.sqrt(a_b_2 + pow(alpha_9 - a_h, 2)));

    var phi_5pr =
      (alpha_5 + a_h + Math.sqrt(a_b_2 + pow(alpha_5 + a_h, 2))) /
      (alpha_2_3 + Math.sqrt(a_b_2 + pow(alpha_2_3, 2)));
    var phi_6pr =
      (alpha_6 + a_h + Math.sqrt(a_b_2 + pow(alpha_6 + a_h, 2))) /
      (alpha_2_3 + Math.sqrt(a_b_2 + pow(alpha_2_3, 2)));
    var phi_7pr =
    ...
    var K_7pr_A_alpha = unlog10(-1.079 - 0.02 * logW);
    ...
    if (logW == Math.round(logW) || kt == 1000) {
      ...
      var Y_8 =
        Y_8_1 + (Y_8_2 - Y_8_1) * (unlog10(logW) / unlog10(Math.ceil(logW)));
    }
  ...
  this.SFSS_fallout = function (kt, rad_doses, fission_fraction, windspeed) {
    ...
    for (var i = 0; i < rad_doses.length; i++) {
      ...
      var d = {
        ...
        draw_stem: rad <= p.i2 ? true : false, //quick test to see if we are above the stem threshhold anyway
        draw_cloud: rad <= p.i7 ? true : false, //ditto for cloud -- numbers above these values produce nonsense results
        max_cloud_rad: p.i7 / (1 / fission_fraction), //this allows us to report what the maximum mappable radiation level is for this yield -- note we take into account fission fraction here
        max_stem_rad: p.i2 / (1 / fission_fraction), //ditto
        x_center: p.x2 * ft2mi,
        ...
        upwind_cloud_distance:
          (rad > p.i6
            ...
            : log_lerp(p.x5, p.i5, p.x6, p.i6, rad)) * ft2mi,

        downwind_cloud_distance: log_lerp(p.x7, p.i7, p.x9, p.i9, rad) * ft2mi,

        max_cloud_width:
          ((p.y8 * log10(p.i7 / rad)) / log10(p.i7 / p.i9)) *
      ...
      if (d.draw_stem) {
        d.stem_area =
          ...
          Math.PI *
          ((d.downwind_stem_distance - d.upwind_stem_distance) / 2) *
          (d.x_center - d.upwind_stem_distance);
      } else {
        d.stem_area = 0;
      }
      if (d.draw_cloud) {
        d.cloud_area =
          //cloud ellipse 1
          (Math.PI * d.downwind_cloud_distance * d.max_cloud_width) / 2 +
          //cloud ellipse 2
          (Math.PI * d.downwind_cloud_distance -
            (d.cloud_widen_point / 2 - d.upwind_cloud_distance / 2) *
              d.max_cloud_width) /
            2;
      } else {
        d.cloud_area = 0;
  ...
  this.SFSS_fallout_points = function (f, angle, steps) {
    ...
    if (f.draw_stem) {
      ...
      if (
        f.upwind_cloud_distance + f.x_center < f.downwind_stem_distance &&
        f.draw_cloud
      ) {
        var pp = [];
        draw_arc(
          pp,
          f.upwind_cloud_distance + f.x_center,
          0,
          f.cloud_widen_point + f.x_center,
          f.max_cloud_width,
          steps
        );
        ...
        p.push([f.downwind_stem_distance * 0.8, f.max_stem_width]);
    ...
    if (f.draw_cloud) {
      ...
      draw_arc(
        ...
        f.cloud_widen_point + f.x_center,
        f.max_cloud_width,
        f.downwind_cloud_distance - f.x_center * 2,
        0,
        steps
      );
      draw_arc(
...
function plot_fallout(gz, points, color, legend) {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    var lng =
      gzlng +
      rad2deg * ((points[i][0] * mi2km) / R / Math.cos(deg2rad * gzlat));
          ...
          fallout_points.push(
            new google.maps.Marker({
              ...
              title:
                ...
                i +
                ":(" +
                Math.round(points[i][0], 2) +
                "," +
                Math.round(points[i][1], 2) +
                ")",
            })
          );
          break;
  ...
  switch (MODE) {
    case GMAP:
      fallout_contours[det_index].push(
      ...
      break;
    case LLET:
      //create a pane for the fallout -- the zIndex seems to work out OK
      if (typeof map.getPane("falloutPane") == "undefined") {
        map.createPane("falloutPane");
        map.getPane("falloutPane").style.zIndex = 390;
      }

      fallout_contours[det_index].push(
        new L.polygon(coords, {
          stroke: false,
          color: color,
          opacity: 1,
          weight: 0,
          ...
          fillOpacity: 0.25,
          zIndex: -10 * (-det_index + 1) + zoff,
          pane: "falloutPane", //keeps it below the dets, because zIndex not supported for polygons apparently?
        }).addTo(map)
      );
      break;
  }
...
function plot_fallout_GE(gz_lat, gz_lng, alt, points, color, legend) {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    var lng =
      gz_lng +
      rad2deg * ((points[i][0] * mi2km) / R / Math.cos(deg2rad * gz_lat));
...
function do_fallout(
  ...
  if (kt < 1 || kt > 10 * pow(10, 5)) {
    if (fallout_info_div_id !== undefined) {
      if (document.getElementById(fallout_info_div_id)) {
        document.getElementById(fallout_info_div_id).innerHTML =
          "<hr><b>The fallout model only works for yields between 1 and 100,000 kilotons.</b>";
      }
    }
    return false;
  }
  ...
  if (!windsock_marker) {
    //create the wind marker
    new_windsock(angle);
  } else if (angle) {
    switch (MODE) {
      case GMAP:
        var m_lat = marker.getPosition().lat(),
          m_lng = marker.getPosition().lng(),
          w_lat = windsock_marker.getPosition().lat(),
          w_lng = windsock_marker.getPosition().lng();
        break;
      case LLET:
        var m_lat = marker.getLatLng().lat,
          m_lng = marker.getLatLng().lng,
          w_lat = windsock_marker.getLatLng().lat,
          w_lng = windsock_marker.getLatLng().lng;
        break;
    }
    windsock_distance = distance_between(m_lat, m_lng, w_lat, w_lng);
    var wpos = destination_from_bearing(
      m_lat,
      m_lng,
      parseInt(angle) + 180,
      windsock_distance
    ...
    switch (MODE) {
      case GMAP:
        ...
        break;
      case LLET:
        windsock_marker.setLatLng([wpos[0], wpos[1]]);
        break;
    }
  }
  switch (MODE) {
    case GMAP:
      if (!angle)
        angle = fallout_bearing(
          marker.getPosition(),
          windsock_marker.getPosition()
        );
      break;
    case LLET:
      if (!angle)
...
function do_fallout_GE(
  ...
  if (kt < 1 || kt > 10 * pow(10, 5)) {
    if (fallout_info_div_id !== undefined) {
      if (document.getElementById(fallout_info_div_id)) {
        document.getElementById(fallout_info_div_id).innerHTML =
          "<hr><b>The fallout model only works for yields between 1 and 100,000 kilotons, sorry.</b>";
      }
    }
    return false;
  }
...
function draw_fallout() {
  ...
  if (fallout_current) {
    ...
    if (fallout_current.fallout_info_div_id) {
      var fallout_info = document.getElementById(
        fallout_current.fallout_info_div_id
      );
    }
    ...
    if (fallout_current.airburst && fallout_current.kt_frac == 0) {
      if (fallout_info) {
        ...
        o += "<hr>";
        o +=
          "<p><b>Fallout:</b> Your choice of burst height is too high to produce significant local fallout. The minimum burst height to produce appreciable fallout for a yield of " +
          ktOrMt(fallout_current.kt) +
          " is " +
          distance(180 * Math.pow(fallout_current.kt, 0.4) * ft2km) +
          ".";
      ...
      if (windsock_marker) {
        ...
        var wpos = destination_from_bearing(
          m_lat,
          m_lng,
          parseInt(fallout_current.angle) + 180,
          windsock_distance
        );
        switch (MODE) {
          case GMAP:
            windsock_marker.setPosition(
              new google.maps.LatLng(wpos[0], wpos[1])
            );
            break;
          case LLET:
            windsock_marker.setLatLng([wpos[0], wpos[1]]);
      ...
      if (fallout_current.kt_frac && fallout_current.airburst) {
        var sfss = fo.SFSS_fallout(
          fallout_current.kt_frac,
          fallout_current.rad_doses,
          fallout_current.fission_fraction / 100,
          fallout_current.wind
        );
      } else {
        var sfss = fo.SFSS_fallout(
          fallout_current.kt,
      ...
      if (fallout_info) {
        ...
        o += "<hr><div>";
        o +=
          "Estimated total-dose <b>fallout contours</b> for a " +
          ktOrMt(fallout_current.kt);
        ...
        if (fallout_current.fission_fraction < 100) {
          o +=
            " (" +
            (fallout_current.fission_fraction > 1
              ? Math.round(fallout_current.fission_fraction)
              : fallout_current.fission_fraction) +
            "% fission)";
        }
      ...
      for (var i = 0; i < fallout_current.rad_doses.length; i++) {
        ...
        plot_fallout(
          pos,
          fo.SFSS_fallout_points(ss, fallout_current.angle, steps),
          f_color,
          fallout_current.rad_doses[i] + " r/hr"
        );

        if (fallout_info) {
          o +=
            "<p><div class='legendkey falloutlegendkey' style='background-color: #" +
            blend_colors(
              blend_colors("000000", background_color, 0.25),
              f_color,
              0.25
            ) +
            "; border: 1px solid #ff5d2e;'></div> Fallout contour for <b>" +
            addCommas(fallout_current.rad_doses[i]) +
            "</b> rads per hour: ";
          o += "<br><ul>";
          ...
          if (ss.draw_cloud || ss.draw_stem) {
            if (!ss.draw_cloud && ss.draw_stem) {
              o +=
                "<li>Maximum downwind cloud (stem only) distance: " +
                distance(ss.downwind_stem_distance * mi2km, false);
              o +=
                "<li>Maximum stem width: " +
                distance(ss.max_stem_width * 2 * mi2km, false);
              o +=
                "<li>Approximate area affected: " +
                distance(ss.stem_area * sqmi2sqkm, false, true);
              o +=
                "<li>The selected radiation level is too high for cloud fallout at this yield, and so this contour is not mapped. Maximum radiation contour for cloud fallout that can be mapped for this yield is " +
                addCommas(Math.round(ss.max_cloud_rad)) +
                " r/hr.";
            } else if (ss.draw_cloud && !ss.draw_stem) {
              o +=
                "<li>Maximum downwind cloud distance: " +
                distance(ss.downwind_cloud_distance * mi2km, false);
              o +=
                "<li>Maximum width: " +
                distance(ss.max_cloud_width * 2 * mi2km, false);
              o +=
                "<li>Approximate area affected: " +
                distance(ss.stem_area + ss.cloud_area * sqmi2sqkm, false, true);
              o +=
                "<li>The selected radiation level is too high for stem fallout at this yield, and so this contour is not mapped. Maximum radiation contour for stem fallout that can be mapped for this yield is " +
                addCommas(Math.round(ss.max_stem_rad)) +
                " r/hr.";
            } else {
              o +=
                "<li>Maximum downwind cloud distance: " +
            ...
            o +=
              "<li>The selected radiation level is too high for fallout at this yield, and so this contour is not mapped. Maximum radiation contour for cloud fallout that can be mapped for this yield is " +
              addCommas(Math.round(ss.max_cloud_rad)) +
              " r/hr; for stem fallout it is " +
              addCommas(Math.round(ss.max_stem_rad)) +
              " r/hr.";
          }
          o += "</ul>";
      ...
      if (fallout_info) {
        if (det_index > 0) {
          ...
          if (fallouts > 1)
            o +=
              "<small>You have multiple fallout contours plotted. The above information is valid only for the active contour.</small><br>";
        }
        if (fallout_current.kt_frac && fallout_current.airburst) {
          o +=
            "<small>*At a burst height of " +
            distance(fallout_current.hob_ft * ft2km) +
            ", the fallout is equivalent to a " +
            ktOrMt(Math.round(fallout_current.kt_frac)) +
            " surface burst with the same fission fraction.</small><br>";
        }
        ...
        if (windsock_marker) {
          o +=
            "<small id='fallout_windsock'>Fallout windsock is " +
            distance(distance_between(m_lat, m_lng, w_lat, w_lng), false) +
            " from ground zero. <a href='#' onclick='hide_windsock(); return false;' id='hide_windsock'>Click here to hide the windsock.</a></small><br>";
        } else {
          o +=
            "<small id='fallout_windsock'><a href='#' onclick='hide_windsock(); return false;' id='hide_windsock'>Click here to show the windsock.</a></small><br>";
        }
        o +=
          "<small>To change the radiation doses to map, <a href='#' onclick='change_doses();return false;'>click here</a>.</small><br>";

        o +=
          '<small>For more information on the fallout model and its interpretation, <a href="faq#fallout" target="_blank">click here</a>.</small>';
        o += "</div></div>";
        fallout_info.innerHTML = o;
      }
...
function draw_fallout_GE() {
  ...
  if (fallout_current) {
    ...
    if (fallout_info) {
      ...
      o +=
        "Estimated total-dose <b>fallout contours</b> for a " +
        addCommas(fallout_current.kt) +
        " kt surface burst" +
        (fallout_current.airburst ? "*" : "");
    ...
    for (var i = 0; i < fallout_current.rad_doses.length; i++) {
      var f_color = colorStep(
        log10(fallout_current.rad_doses[i]),
        log10(1000),
        "FFFF00",
        "FF0000",
        "010000",
      ...
      var fallout_polygon = plot_fallout_GE(
        lat,
        lng,
        fallout_alt,
        fo.SFSS_fallout_points(ss, fallout_current.angle, steps),
      ...
      clouds[det_index].falloutContours.push(
        "det-" + det_index + "-fallout-" + fallout_current.rad_doses[i]
      );
      fallout_polygon.setName(
        "det-" + det_index + "-fallout-" + fallout_current.rad_doses[i]
      );
    ...
    if (fallout_info) {
      o +=
        "<hr><small>*NOTE: The fallout shown is for a <u>surface</u> burst, not an airburst. Airbursts produce almost no fallout of significance if their fireball does not come into contact with the ground.</small>";
      fallout_info.innerHTML = o;
    }
  ...
  if (windsock_marker) {
    switch (MODE) {
      case GMAP:
        ...
        if (fallout_drag_listener)
          google.maps.event.removeListener(fallout_drag_listener);
        break;
      case LLET:
    ...
    document.getElementById("hide_windsock").innerHTML =
      "Click here to show the windsock.";
  } else {
    new_windsock();
    document.getElementById("hide_windsock").innerHTML =
      "Click here to hide the windsock.";
  }
}
...
function new_windsock(angle) {
  ...
  switch (MODE) {
    case GMAP:
      ...
      if (fallout_drag_listener)
        google.maps.event.removeListener(fallout_drag_listener);
      if (marker_drag_listener)
        google.maps.event.removeListener(marker_drag_listener);
      var pos = marker.getPosition();
      var m_lat = marker.getPosition().lat(),
        m_lng = marker.getPosition().lng();
      break;
  ...
  switch (MODE) {
    case GMAP:
      windsock_marker = new google.maps.Marker({
        ...
        draggable: true,
        icon: {
          url: assets + "images/windsock2.png",
      ...
      fallout_drag_listener = google.maps.event.addListener(
        windsock_marker,
        "dragend",
        function () {
          var new_angle = fallout_bearing(
            marker.getPosition(),
            windsock_marker.getPosition()
          );
          ...
          if (document.getElementById("fallout_windsock")) {
            document.getElementById("fallout_windsock").innerHTML =
              "Fallout windsock is " +
              distance(
                distance_between(
                  marker.getPosition().lat(),
                  marker.getPosition().lng(),
                  windsock_marker.getPosition().lat(),
                  windsock_marker.getPosition().lng()
                ),
                false
              ) +
              " from ground zero. <a href='#' onclick='hide_windsock(); return false;' id='hide_windsock'>Click here to hide windsock.</a>";
          }
          update_permalink();
        }
      );
    ...
    case LLET:
      var windIcon = L.icon({
        iconUrl: assets + "images/windsock2.png",
        iconSize: [30, 38],
        iconAnchor: [8, 35],
      });

      //create the marker
      windsock_marker = L.marker(new L.LatLng(wpos[0], wpos[1]), {
        draggable: true,
        title: "Drag me to change wind direction for fallout",
        autoPan: true,
        icon: windIcon,
          ...
          document.getElementById("fallout_windsock").innerHTML =
            ...
            distance(
              distance_between(
                ...
                windsock_marker.getLatLng().lat,
                windsock_marker.getLatLng().lng
              ),
              false
            ) +
            " from ground zero. <a href='#' onclick='hide_windsock(); return false;' id='hide_windsock'>Click here to hide windsock.</a>";
        ...
        if (windsock_marker)
          windsock_marker.setPosition(
            new google.maps.LatLng(
              windsock_marker.getPosition().lat() + dlat,
              windsock_marker.getPosition().lng() + dlng
            )
          );
        break;
      case LLET:
        var dlat = marker.getLatLng().lat - old_marker_pos.lat;
        var dlng = marker.getLatLng().lng - old_marker_pos.lng;
        if (windsock_marker)
          windsock_marker.setLatLng([
            windsock_marker.getLatLng().lat + dlat,
            windsock_marker.getLatLng().lng + dlng,
          ]);
        break;
    }
  }
...
function change_doses() {
  ...
  var input_doses = window.prompt(
    "To change the doses plotted in the fallout curves, enter them in below as numbers separated by commas. These numbers are in rads/hour, between 1 and 30,000. Invalid numbers will be ignored.",
    current
  );
...
function h1_dose_at_point(sample_lat, sample_lon, gz_lat, gz_lon) {
  ...
  var rel_bearing = Math.abs(
    normalize_bearing(fallout_current.angle - 180) - abs_bearing
  );
  ...
  if (
    !point_in_poly(
      ...
      points_to_ll(
        gz_lat,
        gz_lon,
        fo.SFSS_fallout_points(sfss[1], fallout_current.angle, steps)
      )
    )
  ) {
    rad_hr = 0;
    ...
    if (is_inside_contour(max, sample_lat, sample_lon, gz_lat, gz_lon, steps)) {
      ...
      rad_hr = search_between_contours(
        1,
        max,
        sample_lat,
        sample_lon,
...
function search_between_contours(
  low,
  high,
  sample_lat,
  sample_lon,
  ...
  if (s1 && !s2)
    return search_between_contours(
      low,
      half,
      sample_lat,
      sample_lon,
  ...
  if (
    !point_in_poly(
      ...
      points_to_ll(
        gz_lat,
        gz_lon,
        fo.SFSS_fallout_points(sfss[rad], fallout_current.angle, steps)
      )
    )
  ) {
    return 0;
...
function dump_polygon(steps) {
  ...
  for (i in doses) {
    console.log(i + " r/hr:");
    var p = get_latlon_poly(
      dt.pos,
      fo.SFSS_fallout_points(doses[i], fallout_current.angle, steps)
    );
    var o = "";
    for (z in p) {
      o += p[z][0] + ", " + p[z][1] + "\n";
...
let fallout_test = new fallout();

let output = fallout_test.SFSS_fallout(5000, [1, 100, 1000, 10000], 0.5, 15);

let points = fallout_test.SFSS_fallout_points(output, 0, 100);

// console.log(output);

FileSystem.writeFile("fallout_output.json", JSON.stringify(output), (error) => {
